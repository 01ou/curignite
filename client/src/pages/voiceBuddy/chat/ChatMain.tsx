...
const getSelectAction = (situation: Situation): ActionChoices[] => {
  return (ActionsInChatData[situation] ?? []).map((data) => ({ id: data.id as ActionType ?? "startingEffort", text: data.text }));
}

const ChatMain: React.FC<ChatMainProps> = ({}) => {
  const { callbackStatus, callDelayCallbacks } = useDelayCallback();
  const { onAccess, setSituationState, getSituationState, handleStartEffort, isProgressEffort } = useUserStorage();
  const { chats, handleSendText, handleSendPartnerMessage } = useChatManager({ intimacy: 3, onAccess });

  const [currentSituation, setCurrentSituation] = useState<Situation>(getSituationState());
  ...
  return (
    <>      
      <Stack direction="column" alignItems="center" sx={{ overflow: "auto", pb: 20 }} >
        <Typography variant='h6'>
          {todayLabel}
        </Typography>
        {todayChats.map((chat, index) => (
          <MessageBubble key={index} chat={chat} sx={chat.senderId === "self" ? { ml: "auto" } : { mr: "auto" }} />
        ))}
      ...
      <AnimatedPageTransition startTime={effortStartTime} path="/voice-buddy/next" />
    </>
  );
};
