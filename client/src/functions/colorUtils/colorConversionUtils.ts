 ...
 */
export function hexToRgbElements(hex: string): { r: number; g: number; b: number } {
  // 先頭の '#' を削除
  hex = hex.replace(/^#/, '');
 ...
 */
export function getRgbElements(color: ColorFormat): { r: number; g: number; b: number } {
  if (color.startsWith("#")) {
 ...
 */
export function detectColorFormat(color: string): "RGB" | "RGBA" | "Hex" | null {
  if (/^#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/.test(color)) {
    return "Hex";
  } else if (/^rgb\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*\)$/.test(color)) {
    return "RGB";
  } else if (/^rgba\(\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*\d{1,3}\s*,\s*(0|1|0?\.\d+)\s*\)$/.test(color)) {
    return "RGBA";
  }
  return null;
}
 ...
 */
export function convertToRgba(color: ColorFormat, alpha: number = 1): RGBA | null {
